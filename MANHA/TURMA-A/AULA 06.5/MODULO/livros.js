...
const getLivros = function(nomeLivro){
    let nome = String(nomeLivro).toUpperCase();
    let arrayLivros = [];
    let listaDeLivros = {};
    let status = false;

    livros.biblioteca.forEach(function(genero){
        genero.books.forEach(function(item){
            if(item.title.toUpperCase().includes(nome)){
                // console.log(item.title);
                arrayLivros.push({titulo : item.title, isbn: item.isbn13, valor : item.price});
                status = true;
            }
        });
    });
    ...
    if(status){
        listaDeLivros.quantidade = arrayLivros.length;
        listaDeLivros.livros = arrayLivros;
